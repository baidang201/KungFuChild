...
bool GameLayer::init()
{
	...
	String* comboName = String::createWithFormat("comboBtn%d.png", m_iSelectGate);
	...
	auto m_pComboBG = Scale9Sprite::createWithSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(comboName->getCString()));
	m_pComboBtn = ControlButton::create(m_pComboBG);
	m_pComboBtn->setPreferredSize(Sprite::createWithSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(comboName->getCString()))->getContentSize());
	...
	m_pJumpBtn->setPosition(ccp(m_pJumpPicSize.width / 2 - 1.5, m_pJumpPicSize.height / 2));
	...
	pauseGameItem->setPosition(WINSIZE.width - 50, WINSIZE.height - 48);
...
void GameLayer::fistAttack(Ref * pSender, Control::EventType type)
{
	...
	PLAYEFFECT;
	m_pHero->AttackAnimatiion("fist", 0.1f, m_bDirection);
}
...
void GameLayer::footAttack(Ref * pSender, Control::EventType type)
{
	...
	PLAYEFFECT;
	m_pHero->AttackAnimatiion("leg", 0.1f, m_bDirection);
}
...
{
	if (m_pHero->IsAttack || m_pHero->m_bIsJumping || m_pHero->m_bCanCrazy)
	{
		return;
	}
	
	PLAYEFFECTADV("combo.wav");
	m_pHero->m_bCanCrazy = true;

	switch (m_iSelectGate)
	{
	case 1:
		m_pHero->AttackAnimatiion("combo", 0.1f, m_bDirection);
		break;
	case 2:
		m_pHero->AttackAnimatiion("bakandao", 0.1f, m_bDirection);
		break;
	case 3:
		m_pHero->AttackAnimatiion("gun", 0.1f, m_bDirection);
		break;
	default:
		break;
	}
...
void GameLayer::gamePause(Ref * pSender)
{
	...
	//3.0截屏要截皖屏下一帧才能处理RenderTexture。  3.10呢？
...
void GameLayer::gameOver(float delta)
{
	...
	auto m_pEneryLabel = LabelTTF::create("- 1", "", 45);
	m_pEneryLabel->setColor(ccc3(0, 255,255));
	m_pEneryLabel->setPosition(300, 60);
...
void GameLayer::gameVictory(float delta)
{
	...
	moneyTitle->setPosition(WINSIZE.width / 2, WINSIZE.height / 2 - 27);
...
{
	bool hero_dire = hero->HeroDirection;
	bool monster_dire = monster->MonsterDirection;
...
void GameLayer::update(float delta)
{
	...
	if (m_bJump)
	{
		...
		{
			m_pHero->JumpDownAnimation("jumpup", 0.1f, m_bDirection);

			if (m_bDirection == false)
			{
				if (m_pHero->getPositionX() <= WINSIZE.width - 8)//不超出右边
				{
					//精灵到中间或边缘才移动
					if (!m_pHero->JudgePositon(WINSIZE) || myMap->JudgeMap(m_pHero))
					{
						m_pHero->setPosition(ccp(m_pHero->getPositionX() + m_pHero->m_iSpeed, m_pHero->getPositionY() + velocity));
					}
					else
					{
						m_pHero->setPosition(ccp(m_pHero->getPositionX(), m_pHero->getPositionY() + velocity));
					}
		...
		{
			m_pHero->JumpEnd();
			m_pHero->setPosition(ccp(m_pHero->getPositionX()+3, 360));
			velocity = 10.0f;
			m_bJump = false;
		}
	}
	...
	if (m_bRun && m_bDirection == false && m_pHero->IsHurt == false && m_pHero->IsAttack == false && m_pHero->m_bIsJumping == false)
	{
		...
		if (m_pHero->getPositionX() <= WINSIZE.width - 8)//不超出右边
		{
			...
			if (!m_pHero->JudgePositon(WINSIZE) || myMap->JudgeMap(m_pHero))
			{
				m_pHero->setPosition(ccp(m_pHero->getPositionX() + m_pHero->m_iSpeed, m_pHero->getPositionY() ));
			}
			myMap->MoveMap(m_pHero);
		}
	}
	else if (m_bRun && m_bDirection == true && m_pHero->IsHurt == false &&  m_pHero->m_bIsJumping == false)
	{
		...
		if (m_pHero->getPositionX() >= 8)//不超出左边
		{
			if (!m_pHero->JudgePositon(WINSIZE) || myMap->JudgeMap(m_pHero))
			{
				m_pHero->setPosition(ccp(m_pHero->getPositionX() - m_pHero->m_iSpeed, m_pHero->getPositionY()));
	...
	if (m_pHero->IsAttack )
	{
		for (auto monster : monsterShowList)
		{
			if (monster->IsDead == false && isAttackMonster(m_pHero, monster) && m_pHero->m_bIsJumping == false)
			{
				...
				if (dis <= 150)
				{
					switch (monster->m_iType)
					{
					case 1:
						monster->HurntAnimation("monsterHurt", monster->MonsterDirection, 0.2f, 1);
						break;
					case 2:
						monster->HurntAnimation("lionHurt", monster->MonsterDirection, 0.2f, 1);
						break;
					case 3:
						monster->HurntAnimation("stoneHurt", monster->MonsterDirection, 0.2f, 1);
						break;
					default:
						break;
					}
	...
	for (auto monster : monsterShowList)
	{
		if (!monster->IsDead)
		{
			if (monster->IsAttack && m_pHero->IsDead == false && m_pHero->m_bIsJumping == false)
			{
				...
				if (dis <= 130)
				{
					m_pHero->HurtByMonsterAnimatioin("hurt", 0.2f, m_bDirection);
					m_pHPBar->setPercentage(m_pHero->percentage);//掉血
				}
			}
		}
	}
	...
	if (m_pHero->IsDead)
	{
		this->scheduleOnce(schedule_selector(GameLayer::gameOver), 2.0f);
